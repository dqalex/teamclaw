...
OpenClaw Cron Job（定时触发）
  → Gateway sendChatMessage(sessionKey: "cron:teamclaw-check-progress", message: "执行心跳任务：check-progress")
    → Agent 收到消息，读取本模板
      → Agent 通过外部 MCP API 调用工具执行巡检
```

- **Cron Job** 在 OpenClaw Gateway 中配置（Settings → Cron），定义执行频率
- **cron:teamclaw-check-progress** 是独立的心跳会话键，每次运行生成新的 sessionId，不污染主会话
- Agent 收到消息后按本模板步骤执行
...
TeamClaw 在 workspace 目录下自动维护以下文件，心跳巡检时可直接读取：

| 文件 | 格式 | 用途 | 维护方 |
|------|------|------|--------|
| `tasks/TODO.md` | Markdown | **我的任务清单**（待处理 + 进行中，按优先级排序） | TeamClaw 心跳自动写入 |
| `.teamclaw-index` | YAML | 文件索引（路径→ID/hash/version 映射）、心跳状态、同步模式 | TeamClaw 自动写入 |
| `CLAUDE.md` | Markdown | 协作规范 + 动态数据段（项目列表、成员列表、任务统计） | TeamClaw 自动写入 |
| `HEARTBEAT.md` | Markdown | 心跳清单，引用 tasks/ 下的任务文件 | TeamClaw 初始化 + 人工可编辑 |

### `tasks/TODO.md` — 首选任务数据源

TeamClaw 心跳每 120 秒自动刷新，包含分配给你的所有活跃任务（`todo` + `in_progress`）：
...
在 `<!-- TEAMCLAW_DYNAMIC_START -->` 和 `<!-- TEAMCLAW_DYNAMIC_END -->` 标记之间，包含：
...
**首选**：直接读取 workspace 中的 `tasks/TODO.md` 文件（TeamClaw 心跳自动维护，每 120 秒刷新）。
...
如果我有绑定的 workspace 目录，利用 `.teamclaw-index` 索引文件补充任务上下文：

1. **读取 `.teamclaw-index`**：获取 `files` 段，包含所有已同步文件的 ID、hash、version
2. **对比本地文件 mtime/hash**：
   - hash 相同 → 跳过
   - hash 不同 → 标记为变更文件
...
> 直接读取 `.teamclaw-index` 比调用 scan API 更轻量，适合心跳场景。
...
- 是否有未回答的问题
- workspace 中对应的文件是否有最近修改（通过 `.teamclaw-index` 的 hash 对比）
...
- 优先读取 `.teamclaw-index` 和 `CLAUDE.md`，减少 MCP API 调用
