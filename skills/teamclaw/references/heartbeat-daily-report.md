---
title: 心跳任务：日报生成
description: 每日定时生成工作总结，通过 create_document 写入 TeamClaw Wiki，读取 CLAUDE.md 获取项目/成员上下文
trigger: heartbeat
priority: medium
...
OpenClaw Cron Job（每日定时，建议 18:00 或 23:00）
  → Gateway sendChatMessage(sessionKey: "cron:teamclaw-daily-report", message: "执行心跳任务：daily-report")
    → Agent 收到消息，读取本模板
      → Agent 通过外部 MCP API 生成日报
```

- **Cron Job** 在 OpenClaw Gateway 中配置（Settings → Cron），设置每日执行时间
- **cron:teamclaw-daily-report** 是独立的日报会话键，每次运行生成新的 sessionId，不污染主会话
...
生成日报前，先读取 workspace 中 TeamClaw 自动维护的文件获取上下文：

| 文件 | 读取内容 | 用途 |
|------|---------|------|
| `tasks/TODO.md` | 活跃任务列表 | 获取当前**待处理 + 进行中**任务，含 ID、进度、截止日期 |
| `tasks/DONE.md` | 近 24h 完成列表 | 获取**已完成**任务，含 ID、完成日期、项目名 |
| `CLAUDE.md` | `<!-- TEAMCLAW_DYNAMIC_START -->` 到 `<!-- TEAMCLAW_DYNAMIC_END -->` 段 | 获取项目列表（名称→ID）、成员列表、任务统计 |
| `.teamclaw-index` | `files` 段 | 获取 workspace 中已同步文件的 ID 映射，交付文档时可用 |
...
**直接读取本地文件**（TeamClaw 心跳自动维护，每 120 秒刷新）：
...
- 找到匹配文档 → 使用 `update_document`（文档 ID 可从搜索结果或 `.teamclaw-index` 获取）
...
> `document_id` 从 create_document 的返回结果获取，或从 `.teamclaw-index` 中查找对应文件的 ID。
