---
title: 心跳任务：状态同步
description: 定期将 Agent 当前工作状态、任务队列同步到 TeamClaw 状态面板，利用 .teamclaw-index 索引检测文件变更
trigger: heartbeat
priority: low
...
> 本模板由心跳机制触发，将 Agent 实时状态同步至 TeamClaw 面板。
...
OpenClaw Cron Job（建议每 5-10 分钟触发）
  → Gateway sendChatMessage(sessionKey: "cron:teamclaw-sync-to-teamclaw", message: "执行心跳任务：sync-to-teamclaw")
    → Agent 收到消息，读取本模板
      → Agent 通过外部 MCP API 同步状态
```

- **Cron Job** 在 OpenClaw Gateway 中配置（Settings → Cron），定义执行频率
- **cron:teamclaw-sync-to-teamclaw** 是独立的同步会话键，每次运行生成新的 sessionId，不污染主会话
...
TeamClaw 在 workspace 目录下自动维护以下文件，状态同步时**必须使用**：

| 文件 | 格式 | 关键内容 | 更新频率 |
|------|------|---------|---------|
| `tasks/TODO.md` | Markdown | **我的任务清单**（待处理 + 进行中，含 ID/进度/截止日期） | 心跳间隔（120s），有变更时写入 |
| `.teamclaw-index` | YAML | 文件索引（路径→ID/hash/version）、心跳状态、同步模式 | 心跳间隔（120s） |
...
files:
  documents/报告.md:
    id: abc123                # TeamClaw 文档 ID（用于 update_document、deliver_document）
    hash: xyz                 # 文件内容 SHA-256 hash
    version: 2                # 版本号
```

> **重要**：`.teamclaw-index` 中的 `files` 段是 workspace 文件到 TeamClaw 文档 ID 的映射表。
> 当需要通过 MCP API 更新文档时，从这里获取 `document_id`，无需调用 `search_documents`。

### `CLAUDE.md` 动态数据

在 `<!-- TEAMCLAW_DYNAMIC_START -->` 和 `<!-- TEAMCLAW_DYNAMIC_END -->` 标记之间：
- **8.1 可用项目**：项目名 → ID 映射
- **8.2 可用成员**：成员名 → ID 映射
- **8.3 任务概况**：整体任务统计
...
**首选**：直接读取 `tasks/TODO.md`（TeamClaw 心跳自动维护，每 120 秒刷新）：
...
1. **读取 `.teamclaw-index`** 的 `files` 段，获取已索引文件列表及其 hash
2. **扫描目录**中的 `.md` 文件，获取文件列表和 mtime
3. **对比 hash**：
   - hash 相同 → 跳过
   - hash 不同 → 标记为变更文件
   - 新文件（不在 `.teamclaw-index` 中）→ 标记为新增
4. **有变更时**，通过外部 MCP API 更新文档（从 `.teamclaw-index` 获取 `document_id`）：
...
> 从 `.teamclaw-index` 直接获取 `document_id`，比调用 `search_documents` 更快且更准确。

5. **冲突检测**：如果远端 version 与 `.teamclaw-index` 中记录的 version 不匹配，标记冲突但不覆盖
6. **新文件**（不在 index 中）：通过 `create_document` 创建，TeamClaw 同步后会更新 `.teamclaw-index`

**workspace 未绑定时**（`memberId` 为空）：跳过整个第四步。

### 第五步：检查并更新心跳时间

确保 TeamClaw 侧知道此 Agent 在线：
...
| `.teamclaw-index` 缺失 | ⚠️ workspace [名称] 缺少 `.teamclaw-index` 索引文件，建议触发一次同步 |
...
- **优先读取 `.teamclaw-index` 获取文件 hash 和 document_id**，避免调用 scan/search API
